...
import edu.neu.ece.devicedescriptiongenerator.visitor.COWLClassExpressionVisitor;
import edu.neu.ece.devicedescriptiongenerator.visitor.COWLDataRangeVisitor;
import uk.ac.manchester.cs.owl.owlapi.OWLImportsDeclarationImpl;

/**
 * As one of the most critical classes of the program, DeviceDescription defines
 * ontology generator for generating outputs.
 * 
 * @author Yanji Chen
 * @version 1.0
 * @since 2018-10-01
 *
 */
public class DeviceDescriptionGenerator {
	 ...
	 *            ontology.
	 */
	public DeviceDescriptionGenerator(String rootIRIString, int devNumber, long seed, File outputFile,
		...
		OUTPUT_ONTOLOGY_IRI_IN_STRING = "http://ece.neu.edu/ontologies/DeviceDescription" + devNumber + ".owl";
		ran = new Random(seed);
	...
	/**
	 * Get the number of device descriptions.
	 * 
	 ...
	 */
	public void generateRandomRDFDeviceDescriptionInstances()
			throws OWLOntologyStorageException, OWLOntologyCreationException {
		logger.info("Begin generating RDF device descriptions...");
		...
		OWLAnnotation commentAnno = factory.getOWLAnnotation(factory.getRDFSComment(), factory.getOWLLiteral(
				"A sample of " + devNumber + " device " + ((devNumber == 1) ? "description" : "descriptions")));
	...
	public OWLNamedIndividual createResursiveLinkedRDFNode(OWLClass oc, boolean isFirstRecursion) {
		...
		for (COWLClassImpl subClass : ocImplSet)
			if (subClass.isVisited())
				...
				// ran);
				return subClass.getNamedIndividuals().getLast();
		...
		if (rootClass.getSubClassesandItself().contains(ocImpl1) && isFirstRecursion == false) {
			for (COWLClassImpl cls : ocImpl1.getSubClassesandItself())
				if (!cls.getNamedIndividuals().isEmpty())
					return CollectionUtil.getARandomElementFromList(cls.getNamedIndividuals(), ran);
			return null;
		}
		...
		// of the class.
		if (ran.nextDouble() < 1 - newIndividualProbability && !ocImpl2.getNamedIndividuals().isEmpty()
				&& isFirstRecursion == false)
			return CollectionUtil.getARandomElementFromList(ocImpl2.getNamedIndividuals(), ran);

		// Create new individuals if none of the above conditions satisfy.
		...
		ocImpl2.getNamedIndividuals().add(ind);
